...
public class PersonalInformationFacadeImpl implements PersonalInformationFacade {
    ...
    @Override
    @Transactional
    public Mono<BaseResponse<Void>> changePersonalInfo(ChangePersonalInfoRequest request) {
        return this.personalInformationService.findById(request.getId())
                .switchIfEmpty(Mono.error(new EntityNotFoundException(ErrorCode.PERSONAL_INFORMATION_NOT_FOUND)))
    ...
    private Mono<BaseResponse<PersonalInformationResponse>>  findPersonalInfo(Long userId){
        return  this.personalInformationService.findByUserId(userId)
                .switchIfEmpty(Mono.error(new EntityNotFoundException(ErrorCode.PERSONAL_INFORMATION_NOT_FOUND)))
                .map(personalInformation -> BaseResponse.build(
