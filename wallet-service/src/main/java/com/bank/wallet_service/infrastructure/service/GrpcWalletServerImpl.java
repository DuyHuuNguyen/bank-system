...
import com.bank.wallet_service.application.service.GrpcWalletServer;
...
public class GrpcWalletServerImpl extends WalletServiceGrpc.WalletServiceImplBase {
    ...
    @Override
    public void findWalletOwnerByRequest(WalletOwnerRequest request, StreamObserver<WalletResponse> responseObserver) {
        this.walletService.findWalletCurrencyByUserIdAndWalletId(request.getWalletId(), request.getUserId())
    ...
    @Override
    public void findWalletByRequest(WalletRequest request, StreamObserver<WalletResponse> responseObserver) {
        this.walletService.findWalletById(request.getWalletId())
                .switchIfEmpty(Mono.error(new EntityNotFoundException(ErrorCode.WALLET_NOT_FOUND)))
                .flatMap(this::buildWalletResponse)
                .doOnError(err -> log.error("PIPELINE ERROR !!!", err))
                .doOnNext(ok -> log.info("resp {} ", ok))
    ...
    private Mono<WalletResponse> buildWalletResponse(WalletCurrencyDTO walletCurrencyDTO){
        return Mono.just(WalletResponse.newBuilder()
